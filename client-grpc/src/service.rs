    ...
    T: Send + Clone,
{
    ...
    /// use svc_atc_client_grpc::client::{ReadyRequest, RpcServiceClient};
    /// use svc_atc_client_grpc::{Client, GrpcClient};
    /// use svc_atc_client_grpc::service::Client as ServiceClient;
    /// use tonic::transport::Channel;
    ///
    /// async fn example () -> Result<(), Box<dyn std::error::Error>> {
    ///     let (host, port) = get_endpoint_from_env("SERVER_HOSTNAME", "SERVER_PORT_GRPC");
    ///     let connection = GrpcClient::<RpcServiceClient<Channel>>::new_client(&host, port, "atc");
